...
class AlbumService(repository: AlbumRepository) :
    ...
    fun findAllByUser(userId: Long) = if (repository is AlbumRepository) { repository.findAllByUser(userId)} else {throw IllegalStateException("impossible album service state")}

    fun findByIdForUser(albumId: Long, userId: Long) = if (repository is AlbumRepository) { repository.findByIdForUser(albumId, userId)} else {throw IllegalStateException("impossible album service state")}
